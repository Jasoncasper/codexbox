...
fn watcher_install_plan_registers_rust_launcher_at_logon() {
    let plan = build_watcher_install_plan("C:/Tools/codexbox.exe".into(), 9333);

    assert_eq!(plan.run_value_name, "CodexBoxWatcher");
    assert_eq!(
        plan.run_value,
        "\"C:/Tools/codexbox.exe\" --debug-port 9333"
    );
    assert_eq!(plan.shortcut_name, "CodexBoxWatcher.lnk");
    assert_eq!(plan.shortcut_target, "C:/Tools/codexbox.exe");
    assert_eq!(plan.shortcut_arguments, "--debug-port 9333");
}

#[test]
fn spawn_launcher_command_points_to_silent_binary_only() {
    let command = build_spawn_launcher_command("C:/Tools/codexbox.exe", 9444);

    assert_eq!(command[0], "C:/Tools/codexbox.exe");
