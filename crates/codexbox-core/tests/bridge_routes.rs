...
use codexbox_core::models::{DeleteResult, DeleteStatus, ExportResult, ExportStatus, SessionRef};
use codexbox_core::routes::{
    BridgeContext, BridgeDataService, BridgeRuntimeService, BridgeSettingsService,
    CoreRuntimeService, handle_bridge_request,
};
use codexbox_core::settings::BackendSettings;
...
async fn runtime_status_devtools_repair_and_ads_routes_are_dispatched() {
    ...
    assert_eq!(
        ...
        json!({"status": "ok", "message": "后端已修复", "version": codexbox_core::version::VERSION})
    );
    assert_eq!(
...
async fn user_script_manager_deletes_market_script_metadata_and_rejects_builtin_delete() {
    ...
    let script = codexbox_core::script_market::MarketScript {
        id: "demo".to_string(),
        name: "Demo".to_string(),
    ...
    codexbox_core::script_market::install_market_script_content(
        &manager,
        &script,
        b"window.demo = true;",
    )
...
fn script_market_manifest_filters_invalid_entries() {
    ...
    let manifest = codexbox_core::script_market::parse_market_manifest(raw).unwrap();
...
fn user_script_inventory_includes_market_metadata() {
    ...
    manager
        .record_market_install(&codexbox_core::script_market::MarketScript {
            id: "demo".to_string(),
            name: "Demo".to_string(),
...
fn install_market_script_ignores_checksum_mismatch_and_replaces_existing_file() {
    ...
    codexbox_core::script_market::install_market_script_content(&manager, &script, b"new")
        .unwrap();
