...
pub const DEFAULT_REPOSITORY: &str = "Jasoncasper/codexbox";
pub const DEFAULT_LATEST_JSON_URL: &str =
    "https://github.com/Jasoncasper/codexbox/releases/latest/download/latest.json";
...
pub async fn fetch_latest_release(latest_json_url: &str) -> anyhow::Result<Release> {
    let client =
        crate::http_client::proxied_client(&format!("CodexBox/{}", crate::version::VERSION))?;
    let payload = client
        .get(latest_json_url)
