...
use crate::router::config::ProviderProtocol;
...
pub fn sync_linked_profiles_from_default_db(_profiles: &mut Vec<serde_json::Value>) -> anyhow::Result<usize> {
    anyhow::bail!("CCS import of relay profiles has been removed; use crate::router::config types instead")
}

pub fn sync_linked_profiles_from_db(
    _path: &Path,
    _profiles: &mut Vec<serde_json::Value>,
) -> anyhow::Result<usize> {
    anyhow::bail!("CCS import of relay profiles has been removed")
}

pub fn write_linked_profiles_to_default_db(_profiles: &[serde_json::Value]) -> anyhow::Result<usize> {
    anyhow::bail!("CCS write-back of relay profiles has been removed")
}
...
fn import_from_ccs_value(source_id: &str, name: &str, config: &Value) -> Option<CcsProviderImport> {
    ...
    let config_contents =
        config_from_ccs.unwrap_or_else(|| build_config_toml(&base_url, &api_key, protocol.clone()));
...
fn extract_protocol(config: &Value) -> ProviderProtocol {
...
fn build_config_toml(base_url: &str, api_key: &str, protocol: ProviderProtocol) -> String {
    let wire_api = match protocol {
        ProviderProtocol::Responses => "responses",
        ProviderProtocol::ChatCompletions => "chat",
        ProviderProtocol::Anthropic | ProviderProtocol::Custom => "responses",
    };
    [
...
mod tests {
    ...
    fn imports_direct_base_url_and_api_key_provider() {
        ...
        assert_eq!(providers[0].protocol, ProviderProtocol::Responses);
        assert!(
            providers[0]
                .config_contents
    ...
    fn imports_auth_and_config_object_provider_as_chat_protocol() {
        ...
        assert_eq!(providers[0].protocol, ProviderProtocol::ChatCompletions);
        assert_eq!(
