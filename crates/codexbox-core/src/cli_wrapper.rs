...
const CLI_HOME_DIR: &str = ".codexbox-cli";
...
pub fn install_cli_wrapper_to(
    ...
    std::fs::create_dir_all(codex_home)
        .with_context(|| format!("failed to create CodexBox CLI home {}", codex_home.display()))?;
...
pub fn wrapper_dir_from_roaming(roaming: &Path) -> PathBuf {
    ...
    if roaming_text.contains('\\') && !roaming_text.contains('/') {
        return PathBuf::from(format!("{roaming_text}\\CodexBox"));
    }
    roaming.join("CodexBox")
}
...
fn compile_wrapper(source_path: &Path, wrapper_path: &Path) -> anyhow::Result<()> {
    let csc =
        find_csc().ok_or_else(|| anyhow::anyhow!("未找到 csc.exe，无法编译 CodexBox wrapper"))?;
