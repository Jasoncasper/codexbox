...
pub fn extract_request_context(request_json: &Value) -> crate::router::config::RequestContext {
    ...
    let has_image = if let Some(input) = request_json.get("input") {
        ...
        messages.iter().any(|m| detect_image_content(m))
    } else {
        false
    };
...
fn detect_image_content(value: &Value) -> bool {
...
/// 使用智能路由发起代理请求
pub async fn open_routed_proxy_request(body: &str) -> anyhow::Result<(UpstreamProxyResponse, String)> {
    let request_json: Value = serde_json::from_str(body)?;
    let context = extract_request_context(&request_json);
    
    // 尝试加载路由配置
    let router_config_path = crate::paths::default_app_state_dir().join("routing.toml");
    let decision = if router_config_path.exists() {
    ...
    // 如果目标模型与源模型不同，重写模型名
    let mut chat_request = responses_to_chat_completions(request_json)?;
    if decision.target_model != context.model {
