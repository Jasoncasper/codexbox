...
use codexbox_core::launcher::{
    DefaultLaunchHooks, LaunchHooks, LaunchOptions, launch_and_inject_with_hooks,
};
use codexbox_core::models::{DeleteResult, ExportResult, SessionRef};
use codexbox_core::routes::{BridgeContext, BridgeDataService, BridgeRuntimeService};
...
fn acquire_single_instance_guard() -> anyhow::Result<Option<std::net::TcpListener>> {
    match codexbox_core::ports::acquire_loopback_port_guard(
        codexbox_core::ports::LAUNCHER_GUARD_PORT,
    ) {
        ...
        Err(error) if error.kind() == std::io::ErrorKind::AddrInUse => {
            let _ = codexbox_core::diagnostic_log::append_diagnostic_log(
                ...
                json!({
                    "guard_port": codexbox_core::ports::LAUNCHER_GUARD_PORT
                }),
            );
            Ok(None)
        }
...
async fn notify_manager_when_update_available() -> anyhow::Result<bool> {
    let update =
        codexbox_core::update::check_for_update(codexbox_core::version::VERSION).await?;
...
fn open_manager_with_update_prompt() -> anyhow::Result<()> {
    ...
    #[cfg(windows)]
    {
        command.creation_flags(codexbox_core::windows_create_no_window());
    }
    command
        .spawn()
        .map(|_| ())
...
impl LaunchHooks for LauncherHooks {
    fn resolve_app_dir(
        ...
        settings: &codexbox_core::settings::BackendSettings,
    ...
    async fn load_settings(&self) -> anyhow::Result<codexbox_core::settings::BackendSettings> {
        self.core.load_settings().await
    }

    async fn run_provider_sync(&self) -> anyhow::Result<()> {
        let _ = tokio::task::spawn_blocking(|| codexbox_data::run_provider_sync(None))
            .await
    ...
    async fn launch_codex(
        ...
        debug_port: u16,
        extra_args: &[String],
    ) -> anyhow::Result<codexbox_core::launcher::CodexLaunch> {
        self.core
    ...
    async fn wait_for_codex_exit(
        &self,
        launch: &codexbox_core::launcher::CodexLaunch,
    ) -> anyhow::Result<()> {
    ...
    async fn terminate_codex(&self, launch: &codexbox_core::launcher::CodexLaunch) {
...
impl Default for LauncherDataService {
    fn default() -> Self {
        Self {
            ...
            backup_dir: codexbox_core::paths::default_app_state_dir().join("backups"),
        }
    }
}
...
impl BridgeDataService for LauncherDataService {
    ...
    async fn export_markdown(&self, session: SessionRef) -> anyhow::Result<ExportResult> {
        let export_service =
            codexbox_data::MarkdownExportService::new(Some(self.db_path.clone()));
...
impl LauncherDataService {
    fn storage_adapter(&self) -> codexbox_data::SQLiteStorageAdapter {
        codexbox_data::SQLiteStorageAdapter::new(
            self.db_path.clone(),
            codexbox_data::BackupStore::new(self.backup_dir.clone()),
        )
    }
}
...
impl BridgeRuntimeService for LauncherRuntimeService {
    ...
    async fn reload_user_scripts(&self) -> anyhow::Result<Value> {
        ...
        if let Some(websocket_url) = websocket_url.filter(|_| !bundle.trim().is_empty()) {
            codexbox_core::bridge::evaluate_script(&websocket_url, &bundle).await?;
        }
        self.user_scripts.inventory()
    }
    ...
    async fn open_devtools(&self) -> anyhow::Result<Value> {
        ...
        let targets = codexbox_core::cdp::list_targets(debug_port).await?;
        let target = codexbox_core::cdp::pick_page_target(&targets)?;
        let url = codexbox_core::routes::devtools_url(debug_port, &target.id);
        open_url(&url)?;
        Ok(json!({
            "status": "ok",
            "target_id": target.id,
    ...
    async fn open_manager(&self) -> anyhow::Result<Value> {
        ...
        #[cfg(windows)]
        {
            std::process::Command::new(&manager_path)
                .creation_flags(codexbox_core::windows_create_no_window())
                .spawn()
                .map_err(|error| anyhow::anyhow!("启动管理工具失败：{error}"))?;
        }
        #[cfg(not(windows))]
    ...
    async fn backend_status(&self) -> anyhow::Result<Value> {
        Ok(
            json!({"status": "ok", "message": "后端已连接", "version": codexbox_core::version::VERSION}),
        )
    }
    ...
    async fn codex_model_catalog(&self) -> anyhow::Result<Value> {
        Ok(codexbox_core::model_catalog::read_codex_model_catalog().await)
    }

    async fn ads(&self) -> anyhow::Result<Value> {
        codexbox_core::ads::fetch_ad_list().await
    }

    async fn zed_remote_status(&self) -> anyhow::Result<Value> {
        Ok(codexbox_core::zed_remote::zed_remote_status())
    }

    async fn resolve_zed_remote_host(&self, payload: Value) -> anyhow::Result<Value> {
        Ok(codexbox_core::zed_remote::resolve_ssh_target_response(
            &payload,
        ))
    }

    async fn fallback_zed_remote_request(&self, payload: Value) -> anyhow::Result<Value> {
        Ok(codexbox_core::zed_remote::fallback_open_request_response(
            &payload,
        ))
    }

    async fn open_zed_remote(&self, payload: Value) -> anyhow::Result<Value> {
        Ok(codexbox_core::zed_remote::open_zed_remote(&payload))
    }

    async fn upstream_worktree_status(&self) -> anyhow::Result<Value> {
        Ok(codexbox_core::upstream_worktree::status_response())
    }

    async fn upstream_worktree_defaults(&self, payload: Value) -> anyhow::Result<Value> {
        Ok(codexbox_core::upstream_worktree::defaults_response(
            &payload,
        ))
    }

    async fn upstream_worktree_prepare(&self, payload: Value) -> anyhow::Result<Value> {
        Ok(codexbox_core::upstream_worktree::prepare_response(
            &payload,
        ))
    }

    async fn upstream_worktree_create(&self, payload: Value) -> anyhow::Result<Value> {
        Ok(codexbox_core::upstream_worktree::create_response(
            &payload,
        ))
    }
}
...
async fn try_inject_with_context(
    ...
    let script = codexbox_core::assets::injection_script(helper_port);
    ...
    codexbox_core::bridge::install_bridge(
        websocket_url,
        codexbox_core::bridge::BRIDGE_BINDING_NAME,
        Arc::new(move |path, payload| {
            let ctx = ctx.clone();
            Box::pin(async move {
                Ok(codexbox_core::routes::handle_bridge_request(ctx, &path, payload).await)
            })
        }),
...
fn manager_exe_path() -> PathBuf {
    ...
    dir.join(format!(
        "{}{}",
        codexbox_core::install::MANAGER_BINARY,
        suffix
    ))
}
...
fn default_user_scripts_config_dir() -> PathBuf {
    if cfg!(windows) {
        ...
        if let Some(home) = directories::BaseDirs::new().map(|dirs| dirs.home_dir().to_path_buf()) {
            return home.join("AppData").join("Roaming").join("CodexBox");
        }
    }
    ...
    fn manager_update_prompt_uses_sidecar_manager_binary_name() {
        ...
        assert!(
            path.file_name()
                ...
                .is_some_and(|name| name.contains(codexbox_core::install::MANAGER_BINARY))
        );
    }
}
