...
fn main() {
    ...
    if let (Some(debug_port), Some(helper_port)) = (debug_port, helper_port) {
        codexbox_manager_lib::install_panic_logger();
        ...
        let options = codexbox_core::launcher::LaunchOptions {
            app_dir: app_path.map(std::path::PathBuf::from),
            debug_port,
            helper_port,
            status_store: codexbox_core::status::StatusStore::default(),
        };
        let rt = tokio::runtime::Runtime::new().expect("Failed to create tokio runtime");
        match rt.block_on(codexbox_core::launcher::launch_and_inject(options)) {
            Ok(handle) => {
                let _ = codexbox_core::diagnostic_log::append_diagnostic_log(
                    "launcher.success",
                    serde_json::json!({}),
