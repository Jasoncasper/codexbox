...
use codexbox_core::settings::{BackendSettings, RelayProfile, SettingsStore};
...
pub struct CcsProvidersPayload {
    pub db_path: String,
    pub providers: Vec<codexbox_core::ccs_import::CcsProviderImport>,
}
...
pub fn save_settings(settings: BackendSettings) -> CommandResult<SettingsPayload> {
    ...
    if settings.ccs_link_enabled {
        if let Err(error) = codexbox_core::ccs_import::write_linked_profiles_to_default_db(
        ...
        if !active.linked_ccs_provider_id.trim().is_empty() {
            if let Err(error) =
                codexbox_core::ccs_import::set_current_codex_provider_in_default_db(
...
pub fn load_ccs_providers() -> CommandResult<CcsProvidersPayload> {
    let db_path = codexbox_core::ccs_import::default_ccs_db_path();
    match codexbox_core::ccs_import::list_codex_providers_from_db(&db_path) {
        Ok(providers) => ok(
...
pub fn import_ccs_providers() -> CommandResult<SettingsPayload> {
    ...
    let synced = match codexbox_core::ccs_import::list_codex_providers_from_default_db() {
...
fn normalize_settings_before_save(mut settings: BackendSettings) -> BackendSettings {
    ...
    settings.relay_common_config_contents =
        codexbox_core::relay_config::sanitize_common_config_contents(
            &settings.relay_common_config_contents,
        );
    ...
    for profile in &mut settings.relay_profiles {
        if let Err(error) =
            codexbox_core::relay_config::normalize_relay_profile_for_storage(profile)
        {
            log_manager_event(
    ...
    if !common_config.trim().is_empty() {
        for profile in &mut settings.relay_profiles {
            ...
            match codexbox_core::relay_config::strip_common_config_from_config(
...
fn settings_with_live_ccs_profiles(mut settings: BackendSettings) -> BackendSettings {
    ...
    remove_linked_ccs_profiles_for_local_storage(&mut settings);
    if let Err(error) = codexbox_core::ccs_import::sync_linked_profiles_from_default_db(
...
fn remove_linked_ccs_profiles_for_local_storage(settings: &mut BackendSettings) {
        ...
        .any(|profile| profile.id == settings.active_relay_id)
    {
        settings.active_relay_id = settings
            ...
            .unwrap_or_else(codexbox_core::settings::default_active_relay_id);
    }
}
...
fn relay_join_config_sections(sections: &[&str]) -> String {
    ...
    if sections.is_empty() {
        ...
        codexbox_core::relay_config::normalize_config_text(&format!(
            "{}\n",
            sections.join("\n\n")
        ))
    }
...
pub async fn sync_providers_now() -> CommandResult<Value> {
    let result = tauri::async_runtime::spawn_blocking(|| codexbox_data::run_provider_sync(None))
        .await
...
pub fn repair_backend() -> CommandResult<SettingsPayload> {
    ...
    let message = match codexbox_core::cli_wrapper::ensure_cli_wrapper(&settings) {
...
pub fn relay_status() -> CommandResult<RelayPayload> {
    let status = codexbox_core::relay_config::default_relay_status();
...
pub fn backfill_relay_profile_from_live(
    ...
    match codexbox_core::relay_config::backfill_relay_profile_from_home_with_common(
        &home,
        profile,
...
pub fn extract_relay_common_config(
    ...
    match codexbox_core::relay_config::extract_common_config_from_config(&request.config_contents)
        .and_then(|common_config_contents| {
            let profile_config_contents =
                codexbox_core::relay_config::strip_common_config_from_config(
...
pub async fn test_relay_profile(profile: RelayProfile) -> CommandResult<RelayProfileTestPayload> {
    ...
    match codexbox_core::relay_config::test_relay_profile(&profile, test_model).await {
        Ok(result) => {
...
pub async fn fetch_relay_profile_models(
    ...
    match codexbox_core::model_catalog::fetch_relay_profile_model_ids(&profile).await {
...
pub fn apply_relay_injection() -> CommandResult<RelayPayload> {
    ...
    if !settings.relay_profiles_enabled {
        let status = codexbox_core::relay_config::relay_status_from_home(&home);
        return failed(
    ...
    if relay.config_contents.trim().is_empty() == false
        && relay.relay_mode == codexbox_core::settings::RelayMode::Official
        && relay.official_mix_api_key
    {
        return match codexbox_core::relay_config::apply_relay_profile_config_to_home_with_context(
            &home,
            &relay,
    ...
    if relay_has_complete_files(&relay) {
        return match codexbox_core::relay_config::apply_relay_profile_to_home_with_switch_rules(
            &home,
            &relay,
            &relay_combined_common_config(&settings),
        ) {
    ...
    let auth = codexbox_core::relay_config::chatgpt_auth_status_from_home(&home);
    ...
    match codexbox_core::relay_config::apply_relay_config_to_home_with_protocol(
        &home,
        &relay.base_url,
        &relay.api_key,
        relay.protocol,
        codexbox_core::protocol_proxy::DEFAULT_PROTOCOL_PROXY_PORT,
    ) {
        Ok(result) => {
            let status = codexbox_core::relay_config::relay_status_from_home(&home);
            log_relay_apply_result(
...
pub fn apply_pure_api_injection() -> CommandResult<RelayPayload> {
    ...
    match codexbox_core::relay_config::apply_pure_api_config_to_home_with_protocol(
        &home,
        &relay.base_url,
        &relay.api_key,
        relay.protocol,
...
pub fn clear_relay_injection() -> CommandResult<RelayPayload> {
    ...
    let auth_contents = (relay.relay_mode == codexbox_core::settings::RelayMode::Official
        && !relay.official_mix_api_key
        && !relay.auth_contents.trim().is_empty())
    .then_some(relay.auth_contents.as_str());
    match codexbox_core::relay_config::clear_relay_config_to_home_with_auth(&home, auth_contents)
    {
        Ok(result) => {
            let status = codexbox_core::relay_config::relay_status_from_home(&home);
            log_manager_event(
...
fn relay_has_complete_files(relay: &codexbox_core::settings::RelayProfile) -> bool {
...
fn log_relay_apply_request(
    ...
    relay: &codexbox_core::settings::RelayProfile,
) {
    let _ = codexbox_core::diagnostic_log::append_diagnostic_log(
        event,
        json!({
...
fn log_relay_apply_result(
    event: &str,
    relay: &codexbox_core::settings::RelayProfile,
    status: &codexbox_core::relay_config::RelayStatus,
...
fn log_manager_event(event: &str, detail: Value) {
    let _ = codexbox_core::diagnostic_log::append_diagnostic_log(event, detail);
}
...
fn refresh_cli_wrapper_after_settings_save(settings: &BackendSettings) -> String {
    match codexbox_core::cli_wrapper::ensure_cli_wrapper(settings) {
        Ok(Some(install)) => format!(
            " 命令包装器已更新：{}。",
            install.real_codex.to_string_lossy()
        ),
...
mod tests {
    ...
    fn relay_payload_does_not_expose_token_text() {
        let payload = relay_payload(
            codexbox_core::relay_config::RelayStatus {
                authenticated: true,
    ...
    fn apply_relay_profile_to_home_with_switch_rules_normalizes_custom_provider_id_to_stable_bucket()
     {
        ...
        let profile = RelayProfile {
            relay_mode: codexbox_core::settings::RelayMode::PureApi,
            protocol: codexbox_core::settings::RelayProtocol::Responses,
        ...
        codexbox_core::relay_config::apply_relay_profile_to_home_with_switch_rules(
            temp.path(),
            &profile,
            "",
        )
    ...
    fn normalize_settings_before_save_preserves_official_profile_auth() {
        let settings = BackendSettings {
            relay_profiles: vec![RelayProfile {
                relay_mode: codexbox_core::settings::RelayMode::Official,
