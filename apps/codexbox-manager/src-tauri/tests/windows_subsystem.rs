...
fn macos_packager_hides_silent_launcher_but_not_manager() {
    ...
    assert!(script.contains(
        "create_app \"CodexBox\" \"CodexPlusPlus\" \"$BINARY_DIR/codex-plus-plus\" \"com.bigpizzav3.codexplusplus\" \"true\""
    ));
    assert!(script.contains(
        "create_app \"CodexBox 管理工具\" \"CodexPlusPlusManager\" \"$BINARY_DIR/codexbox-manager\" \"com.bigpizzav3.codexplusplus.manager\" \"false\""
    ));
}
